    ...
    (() => {
        ...
        function tabs() {
            ...
            function setTabsAction(e) {
                const el = e.target;
                if (el.closest(".header-catalog__sub-title")) {
                    const tabTitle = el.closest(".header-catalog__sub-title");
                    const tabsBlock = tabTitle.closest("[data-tabs]");
                    const tabsActiveTitle = tabsBlock.querySelector("[data-tabs-title]._tab-active");
                    if (tabsActiveTitle) tabsActiveTitle.classList.remove("_tab-active");
                } else if (el.closest("[data-tabs-title]")) {
        ...
        class Popup {
                ...
                document.addEventListener("click", function(e) {
                    ...
                    if (buttonOpen) {
                        ...
                        if (this._dataValue !== "error") {
                            ...
                            if (this.targetOpen.element && this.targetOpen.element.classList && this.targetOpen.element.classList.contains(this.options.classes.popupActive)) this.close(); else this.open();
                            return;
            ...
            open(selectorValue) {
                ...
                localStorage.setItem("lastOpenedPopup", this.targetOpen.selector);
            }
            close(selectorValue) {
        ...
        window.addEventListener("load", (function() {
            ...
            const lastOpenedPopup = localStorage.getItem("lastOpenedPopup");
            if (isPopupOpen === "true" && lastOpenedPopup) {
                const popup = new Popup({});
                popup.open(lastOpenedPopup);
            }
        }));
        ...
        function initSliders() {
            const initializeSwiper = (selector, options) => {
                if (document.querySelector(selector)) {
                    const swiperInstance = new swiper_core_Swiper(selector, options);
                    const htmlElement = document.documentElement;
                    const observer = new MutationObserver((mutations => {
                        mutations.forEach((mutation => {
                            if (mutation.type === "attributes" && mutation.attributeName === "class") if (htmlElement.classList.contains("menu-open")) swiperInstance.autoplay.stop(); else swiperInstance.autoplay.start();
                        }));
                    }));
            ...
            initializeSwiper(".hero__slider.hero__slider--first.swiper", {
                ...commonOptions,
                pagination: {
                    ...commonOptions.pagination,
                    el: ".hero__slider.hero__slider--first .swiper-pagination"
                },
                navigation: {
                    prevEl: ".hero__slider.hero__slider--first .swiper-button-prev",
                    nextEl: ".hero__slider.hero__slider--first .swiper-button-next"
                }
            });
            if (window.innerWidth > 991.98) initializeSwiper(".hero__slider.hero__slider--second.swiper", {
                ...commonOptions,
                autoplay: {
                ...
                pagination: {
                    ...commonOptions.pagination,
                    el: ".hero__slider.hero__slider--second .swiper-pagination"
                },
                navigation: {
                    prevEl: ".hero__slider.hero__slider--second .swiper-button-prev",
                    nextEl: ".hero__slider.hero__slider--second .swiper-button-next"
                }
            });
            initializeSwiper(".aside-item__slider._first", {
                ...commonOptions,
                ...
                navigation: {
                    prevEl: ".aside-item .swiper-button-prev",
                    nextEl: ".aside-item .swiper-button-next"
                }
            });
            if (window.innerWidth > 991.98) initializeSwiper(".aside-item__slider._second", {
                ...commonOptions,
                modules: [ Navigation, Autoplay, EffectFade ],
                fadeEffect: {
                    crossFade: true
                ...
                navigation: {
                    prevEl: ".aside-item__slider._second .swiper-button-prev",
                    nextEl: ".aside-item__slider._second .swiper-button-next"
                }
            });
            const productsContainers = document.querySelectorAll(".products");
            productsContainers.forEach((container => {
                const slider = container.querySelector(".products__slider");
                const prevButton = container.querySelector(".swiper-button-prev");
                const nextButton = container.querySelector(".swiper-button-next");
                if (slider && prevButton && nextButton) new swiper_core_Swiper(slider, {
                    ...commonOptions,
                    modules: [ Navigation ],
                    slidesPerView: 1.15,
                    spaceBetween: 10,
        ...
        function initCatalogSearch() {
            const searchOpen = document.querySelector(".catalog-search__open");
            const searchForm = document.querySelector(".catalog-search__form");
            const searchFormClose = document.querySelector(".catalog-search__form-close");
            if (searchOpen && searchForm && searchFormClose) {
                searchOpen.addEventListener("click", (() => searchForm.classList.add("_open")));
                searchFormClose.addEventListener("click", (() => searchForm.classList.remove("_open")));
            }
        }
        function initContactsToggle() {
            const headerContacts = document.querySelector(".header-contacts");
            const bodyWrapper = document.querySelector(".contacts__body-wrapper");
            if (!headerContacts || !bodyWrapper) return;
            function handleResize() {
                if (window.innerWidth <= 991.98) headerContacts.addEventListener("click", toggleBodyWrapper); else {
                    headerContacts.removeEventListener("click", toggleBodyWrapper);
                    bodyWrapper.classList.remove("_open");
                }
            }
            function toggleBodyWrapper(event) {
                if (event.target.classList.contains("contacts__close")) bodyWrapper.classList.remove("_open"); else bodyWrapper.classList.add("_open");
            }
            window.addEventListener("resize", script_debounce(handleResize, 100));
            handleResize();
        }
        function initAddToCartAnimation() {
            const cart = document.querySelector(".header-actions__basket");
            const buttons = document.querySelectorAll(".product__button");
            let cartItemCount = 0;
            const cartItemCountElement = document.querySelector(".header-actions__basket-count");
            if (!cart) return;
            buttons.forEach((button => {
                button.addEventListener("click", (function() {
                    ...
                    const computedStyles = window.getComputedStyle(productCard);
                    const productCardRect = productCard.getBoundingClientRect();
                    const cartRect = cart.getBoundingClientRect();
                    const flyingCard = productCard.cloneNode(true);
                    setFlyingCardStyles(flyingCard, productCardRect, computedStyles);
                    document.body.appendChild(flyingCard);
                    requestAnimationFrame((() => {
                        flyingCard.style.transform = `translate(${cartRect.left - productCardRect.left + 30}px, ${cartRect.top - productCardRect.top + 10}px) scale(0)`;
                        flyingCard.style.opacity = "0";
                    }));
                    flyingCard.addEventListener("transitionend", (() => {
                        setTimeout((() => flyingCard.remove()), 500);
                    }));
                }));
            }));
            function updateCartItemCount() {
                if (cartItemCountElement) {
                    ...
                    if (cartItemCount > 0) cartItemCountElement.classList.add("active"); else cartItemCountElement.classList.remove("active");
                }
            }
        }
        function setFlyingCardStyles(flyingCard, productCardRect, computedStyles) {
            flyingCard.style.position = "fixed";
            flyingCard.style.top = `${productCardRect.top}px`;
            flyingCard.style.left = `${productCardRect.left}px`;
            flyingCard.style.transformOrigin = "top left";
            flyingCard.style.transform = "translate(0, 0)";
            flyingCard.style.transition = "transform 0.7s ease-in-out, opacity 0.7s ease-in-out";
            flyingCard.style.zIndex = "100";
            flyingCard.style.opacity = "0.7";
            flyingCard.style.width = computedStyles.width;
            flyingCard.style.height = computedStyles.height;
            flyingCard.style.backgroundColor = computedStyles.backgroundColor;
            flyingCard.style.color = computedStyles.color;
            flyingCard.style.border = computedStyles.border;
        }
        function script_debounce(func, wait) {
            let timeout;
